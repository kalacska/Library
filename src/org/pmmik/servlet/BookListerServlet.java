	...
	private void action(HttpServletRequest request, HttpServletResponse response) {
		try {
			...
			try {
				...
				if (request.getSession().getAttribute("admin") == null) { //$NON-NLS-1$
					this.page = "/listofbooks.jsp"; //$NON-NLS-1$
				} else {
					this.page = "/adminlistofbooks.jsp"; //$NON-NLS-1$
				}

				RequestDispatcher disp = request
						.getRequestDispatcher(this.page);
