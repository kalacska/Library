	...
	protected void doPost(HttpServletRequest request,
		...
		if (session.getAttribute("admin") == null) { //$NON-NLS-1$
			String htmlMessage = "<!DOCTYPE html>" + //$NON-NLS-1$
					...
					+ "</head>" + "<body bgcolor='#C6EAFF'>"; //$NON-NLS-1$ //$NON-NLS-2$

			if (books.isEmpty()) {
				htmlMessage += "<p align='center' style='font-weight:bold;'>No result!</p>"; //$NON-NLS-1$
			} else {

				htmlMessage += "<h1 align='center'>Search Result</h1>" + //$NON-NLS-1$
				...
				for (Book book : books) {
					htmlMessage += "<tr><td>" + book.getAuthor() + "</td><td>" + book.getTitle() + "</td><td>" + //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
							book.getIsbn()
							+ "</td><td>" + book.isLoanable() + "</td><td>" + book.getAmount() + "</td>"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
				}
			...
			if (books.isEmpty()) {
				...
				htmlMessage += "<form method='post' action='DeleteServlet' accept-charset='ISO-8859-2'>" + //$NON-NLS-1$
				...
				for (Book book : books) {
					htmlMessage += "<tr><td>" + book.getAuthor() + "</td><td>" + book.getTitle() + "</td><td>" + //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
							book.getIsbn()
							+ "</td><td>" + book.isLoanable() + "</td><td>" + book.getAmount() + "</td><td><input type='checkbox' name='deleteCB' value=" + book.getIsbn() + "/></td>"; //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
				}
